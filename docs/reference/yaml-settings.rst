...
- **netmask_groups**: Sequence of :ref:`NetmaskGroupConfiguration <yaml-settings-NetmaskGroupConfiguration>` - Netmask groups definitions
...
------------------------
 ...
 .. versionchanged:: 2.1.0
   The ``rcode`` parameter used to be an unsigned integer and is now a string
...
.. _yaml-settings-NetmaskGroupConfiguration:
...
- **name**: String - The name of this netmask group
- **netmasks**: Sequence of String ``("")`` - List of netmasks. A subnet that is not followed by ``/`` will be interpreted as a ``/32`` or ``/128`` subnet (a single address), depending on address family. It is possible to exclude ranges by prefixing an item with the negation character ``!``
...
- **listen_addresses**: Sequence of String ``("")`` - IP addresses and ports to listen on
