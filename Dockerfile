...
# Unable backports to get a recent enough version of meson(test)
...
# add your local dnsdist-2.0.0 source
#COPY dnsdist-2.0.0/ /source/
...
RUN mkdir /quiche && cd /quiche && \
    ...
    chmod +x install_rust.sh install_quiche.sh && \
    ./install_rust.sh && \
    ./install_quiche.sh
...
RUN for tentative in "lib/aarch64-linux-gnu" "lib64" "lib"; do \
...
COPY dnsdist-2.0.0/dockerdata/dnsdist.conf /etc/dnsdist/

# Start script
COPY dnsdist-2.0.0/dockerdata/startup.py /usr/local/bin/dnsdist-startup
...
COPY dnsdist-2.0.0/dockerdata/dnsdist-resolver.lua /etc/dnsdist/

ENTRYPOINT ["/usr/bin/tini", "--", "python3", "/usr/local/bin/dnsdist-startup"]
